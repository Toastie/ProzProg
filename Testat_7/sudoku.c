...
void printSudoku(){     //Gibt das aktuelle Sudoku aus
    printf("  |  ");
...
int nullenVorhanden(){      //Prüft ob Nullen vorhanden sind, wenn keine vorhanden sind ist das Sudoku gelöst (keine falschen Eingaben möglich)
...
int finja(int x, int y){
    ...
    if(original[x][y] != 0){
        if(finja(xNach,yNach)){
            return(1);
        }
    }

    for (int i = 1; i < 10; i++){
        if (eingabePruefen(x,y,i) == 4){
            sudoku[x][y] = i;
            if(finja(xNach,yNach)){
                return(1);
            }
...
int main(){
    ...
    //Abfrage ob das Sudoku automatisch gelöst werden soll
    printf("Soll das Sudoku automatisch gelöst werden? (1 = yes):");
    scanf("%d",&frage);
    ...
    while(nullenVorhanden(sudoku)){
        ...
        if(x < 9 && y < 9 && x >= 0 && y >= 0 && z < 10 && z > 0){
            //Unterscheidung der unterschiedlichen Fehler um die unzulässigkeit der Eingabe zu begründen
                ...
                case 4: system("@cls||clear"); sudoku[x][y] = z; printSudoku(); break;
                case 5: printf("Dieser Wert darf nicht verändert werden!");
                }
        }
        else{
